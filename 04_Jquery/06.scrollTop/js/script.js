...
$(window).scroll(() => {
    let box = $('.box');
    let sct = $(window).scrollTop();
    let rnd1 = Math.round(Math.random() * 20);
    console.log("🚀 ~ file: script.js:28 ~ $ ~ sct:", sct)

    box.each(function () {
        let boxTop = $(this).offset().top
        console.log("🚀 ~ file: script.js:32 ~ boxTop:", boxTop)
        
        if(sct > boxTop-1800){
            $(this).clearQueue().stop().animate({top: '-50px'}, 1000)
        }else{
            $(this).clearQueue().stop().animate({top: '500px'}, 500)
        }
    })
})
